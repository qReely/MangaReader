...
class AsuraParser extends Parser{
  ...
  Future<void> loadManga() async {
    ...
    print(CurrentBox.asurascans.getUrl());
    Uri url = Uri.parse("${CurrentBox.asurascans.getUrl()}/list-mode/");
    http.Response response = await http.get(url); // if throws error try again
    ...
    for(int page = 1; page <= totalPages; page++){
      List<String> titles = [];
      url = Uri.parse("${CurrentBox.asurascans.getUrl()}/?order=update&page=$page");
  ...
  Future<void> _loadChapters(Manga manga) async {
    ...
    try{
      ...
      List<String> names = [];
      // get only those that are needed ? ? ? ? ?. What if manga has 1000+ chapters
      ...
      // if ongoing - check
      // else check if chapters latest chapter's are the same
